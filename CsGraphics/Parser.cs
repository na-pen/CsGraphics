namespace CsGraphics
{
    ...
    internal static class Parser
    {
        ...
        internal static (double[,], Dictionary<string, int[][]>, Matrix[], Dictionary<string, (Color, string)>, int[][], double[][]) ObjParseVerticesV2(string filePath)
        {
            var vertices = new List<double[]>(); // 動的リストで頂点情報を一時的に格納
            var verticesT = new List<double[]>(); // 動的リストでテクスチャ座標情報を一時的に格納
            ...
            foreach (var line in File.ReadLines(filePath))
            {
                ...
                {
                    var parts = line.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    dic = MtlParserV2(System.IO.Path.GetDirectoryName(filePath) + "\\" + parts[1]);
                }
                else if (line.StartsWith("usemtl ")) // 面情報のとき
                {
                    var parts = line.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            return (vertexArray,
                ...
                normal.ToArray(),
                dic,
                mtlV.Select(innerList => innerList.ToArray()).ToArray(),
                verticesT.ToArray());
        }
